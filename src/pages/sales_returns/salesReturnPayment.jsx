...
const SalesReturnPayment = () => {
  //meta title
  document.title = "Sales Return Payment";
  ...
  const [salePayments, setSalesReturnPayments] = useState([]);
  ...
  const fetchSalesReturnPayments = async () => {
    try {
      setLoading(true);
      const { data } = await axiosInstance.get(
        "salePayments/get_sales_return_payments"
      );
      if (data.success) {
        setSalesReturnPayments(data.salesReturnPayments);
        setLoading(false);
      } else {
        setLoading(false);
      }
  ...
  const fetchInvoices = async (customerId) => {
    try {
      const { data } = await axiosInstance.get(
        "sales/" + customerId + "/returns"
      );
      setInvoices(
        data.invoices.map((invoice) => {
          return {
            label: "SRT" + invoice.id,
            value: invoice.id,
          };
        })
      );
  ...
  const onDeletePayment = async () => {
    try {
      const { data } = await axiosInstance.post(
        "salePayments/delete_sales_return_payment",
        {
          paymentId: saleId,
          userId: user.id,
        }
            ...
            <div className="d-flex gap-3">
              <Link
                ...
                onClick={() => {
                  ...
                  setSelectedInvoice({
                    label: "SRT" + cellProps.row.original.salesReturnId,
                    value: cellProps.row.original.salesReturnId,
                  });
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumbs */}
          <Breadcrumbs title="People" breadcrumbItem="SalesReturnPayment" />
          {isLoading ? (
              ...
              <Form
                onSubmit={async (e) => {
                  ...
                  if (isEdit) {
                    ...
                    try {
                      const { data } = await axiosInstance.post(
                        "salePayments/create_sales_return_payment",
                        newPayment
                      );
                      if (data.success) {
                        toast.success(data.message);
...
export default SalesReturnPayment;
