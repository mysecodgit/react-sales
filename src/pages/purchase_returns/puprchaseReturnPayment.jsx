...
const PurchaseReturnPayment = () => {
  ...
  const [purchasePayments, setPurchaseReturnPayments] = useState([]);
  const [deleteModal, setDeleteModal] = useState(false);

  const [bankAccounts, setBankAccounts] = useState();
  const [setlectedBankAccount, setSelectedBankAccount] = useState(null);

  const fetchPurchaseReturnPayments = async () => {
    try {
      setLoading(true);
      const { data } = await axiosInstance.get(
        "purchasePayments/get_purchase_return_payments"
      );
      if (data.success) {
        setPurchaseReturnPayments(data.purchaseReturnPayments);
        setLoading(false);
      } else {
        setLoading(false);
      }
  ...
  const fetchInvoices = async (vendorId) => {
    try {
      const { data } = await axiosInstance.get(
        "purchases/" + vendorId + "/returns"
      );
      setInvoices(
        data.invoices.map((invoice) => {
          return {
            label: "PRT" + invoice.id,
            value: invoice.id,
          };
        })
      );
  ...
  const onDeletePayment = async () => {
    try {
      const { data } = await axiosInstance.delete(
        "purchasePayments/" + purchaseId + "/delete_purcahse_return"
      );
      if (data.success) {
              ...
              <Form
                onSubmit={async (e) => {
                  ...
                  const newPayment = {
                    ...
                    branchId: null, // because i am using the head quater branch id
                    vendorId: selectedVendor.value,
                    returnId: selectedInvoice.value,
                  ...
                  if (isEdit) {
                    ...
                    try {
                      const { data } = await axiosInstance.post(
                        "purchasePayments/create_purchase_return_payment",
                        newPayment
                      );
                      if (data.success) {
                        toast.success(data.message);
...
export default PurchaseReturnPayment;
